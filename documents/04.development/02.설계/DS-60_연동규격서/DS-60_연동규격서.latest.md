...
| v0.18 | 2026-07-16 | Architect | Redmine #29 **BE 구현 완료 현행화**(박개발, `commands/web.rs`·`error.rs`, cargo test 114/0 PASS): §8.2 `web` capability 주석을 'SSRF 가드 추가 예정' → **'구현 완료(SSRF 전면 차단·redirect 미추종·`URL_BLOCKED`)'**로 전환, §9 `URL_BLOCKED` 설명을 현행 구현 기준으로 갱신, §4.4 상태를 'BE 구현 완료(FE 잔여)'로 명시. DS-40 v0.13과 교차 정합 |
...
> **상태: BE 구현 완료(현행, 2026-07-16)** — `commands/web.rs`(SSRF 가드·Content-Type·`FetchedPage → Document 첨부` 변환), cargo test 114/0 PASS. FE(웹 첨부 진입점·`src/ipc/web.ts` wrapper·web chip)는 잔여 작업.
...
| `web` | 현행 | `fetch_url_content` (Redmine #29 구현 완료: SSRF **전면 차단**·redirect 미추종(`Policy::none`)·Content-Type html/plain 제한, 차단 시 `URL_BLOCKED` — DS-40 §10.7) |
...
| `URL_BLOCKED` | SSRF 가드 차단(구현 완료, #29): 내부망(loopback·link-local·사설·사내) 호스트, 비허용 scheme, redirect 응답 | 외부 공개 URL로 변경 안내 |
